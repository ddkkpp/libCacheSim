...
// ============================================================
// LOH 统一版本 - 支持三种运行模式（通过环境变量切换）
// ============================================================
// 模式1: LOH_ENABLE_PENALTY=1 - 使用penalty队列机制
// 模式2: LOH_WAIT_MODE=blocked（默认）- 等待Python更新权重（阻塞）
// 模式3: LOH_WAIT_MODE=nonblocked - 不等待，使用上次权重继续运行（非阻塞）
// ============================================================
...
#define LOH_INCLUDE_TOPK_CANDIDATE_FEATURES 1
#endif
...
#define CONTEXT_DIM (BASE_STATE_DIM + CAND_FEATURE_DIM + TOPK_FEATURE_DIM)
...
cache_t *LOH_init(const common_cache_params_t ccache_params,
  ...
  if (params->enable_rl) {
    ...
    LOH_DEBUG_PRINT_BASIC("  - CAND_FEATURE_DIM=%d, TOPK_FEATURE_DIM=%d\n",
                          CAND_FEATURE_DIM, TOPK_FEATURE_DIM);
