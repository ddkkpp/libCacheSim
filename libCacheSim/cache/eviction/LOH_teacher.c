...
static void loh_teacher_export_candidates(LOH_params_t *params,
                                          double feat_buf[][FEATURE_DIM],
                                          int cand_n,
                                          cache_obj_t *student_choice);
...
static inline int64_t loh_teacher_belady_key(cache_obj_t *obj) {
  if (obj == NULL) return INT64_MIN;
  int64_t v = obj->misc.next_access_vtime;
  if (v == -1 || v == INT64_MAX) return INT64_MAX;
  return v;
}
...
static int loh_teacher_belady_idx(LOH_params_t *params, int cand_n) {
  ...
  for (int i = 0; i < cand_n; ++i) {
    ...
    int64_t key = loh_teacher_belady_key(obj);
...
static void loh_teacher_export_candidates(LOH_params_t *params,
                                          ...
                                          int cand_n,
                                          cache_obj_t *student_choice) {
  if (params == NULL || !params->teacher_export_enabled ||
  ...
  int student_idx = -1;
  for (int i = 0; i < cand_n; ++i) {
    if (params->candidates[i] == student_choice) {
      student_idx = i;
      break;
    }
  }
  ...
  for (int i = 0; i < cand_n; ++i) {
    ...
    unsigned long long obj_id = obj ? (unsigned long long)obj->obj_id : 0ULL;
    const double *f = feat_buf[i];

    fprintf(params->teacher_export_fp,
            "%llu,%d,%d,%d,%d,%lld,%lld,%llu,%.17g,%.17g,%.17g,%.17g,%.17g,"
            "%.17g\n",
...
cache_t *LOH_teacher_init(const common_cache_params_t ccache_params,
  ...
  cache_t *cache =
      cache_struct_init("LOH-Teacher", ccache_params, cache_specific_params);
  cache->cache_init = LOH_teacher_init;
...

  {
    ...
    if (teacher_export_path && teacher_export_path[0] != '\0') {
      ...
      params->teacher_export_path = g_strdup(teacher_export_path);
      params->teacher_export_fp =
          fopen(params->teacher_export_path, append_mode ? "a" : "w");
      if (params->teacher_export_fp != NULL) {
        params->teacher_export_enabled = 1;
        if (append_mode) {
          if (fseek(params->teacher_export_fp, 0, SEEK_END) == 0) {
            long pos = ftell(params->teacher_export_fp);
            if (pos == 0) {
              fprintf(params->teacher_export_fp,
                      "evict_seq,cand_n,cand_idx,is_teacher,student_pick,next_"
                      "access_vtime,belady_key,obj_id,feature0,feature1,"
...
static void LOH_free(cache_t *cache) {
  ...
  if (params->teacher_export_fp) {
    fflush(params->teacher_export_fp);
    fclose(params->teacher_export_fp);
    params->teacher_export_fp = NULL;
  }
  if (params->teacher_export_path) {
    g_free(params->teacher_export_path);
    params->teacher_export_path = NULL;
  }
...
static cache_obj_t *LOH_to_evict(cache_t *cache, const request_t *req) {
  ...
  size_ds_get_candidates(params, candidates_per_feature, &params->n_candidates);
  ...
  if (loh_score_use_compound) {
    ...
    if (loh_use_heuristic_signs) {
      // 使用启发式符号：Recency/Size/IRT 越大越容易被驱逐（负号），Frequency
      // 越大越保留（正号）
      sign[0] = -1.0;  // Recency
      sign[1] = 1.0;   // Frequency
      sign[2] = -1.0;  // Size
  ...
  loh_teacher_export_candidates(params, feat_buf, cand_n, obj_to_evict);
...
static void size_heap_init(LOH_params_t *params) {
  ...
  params->size_heap_capacity = MAX_CANDIDATES;
...
static void size_buckets_get_candidates(LOH_params_t *params,
  ...
  // 收集所有桶中的对象到临时数组，然后按 size 排序选择
  // 这样可以确保选择的是全局最大的对象，而不是按桶内 LRU 顺序
  cache_obj_t *temp_objs[MAX_CANDIDATES * 2];  // 临时存储，预留足够空间
  int temp_count = 0;

  // 从所有桶收集对象
  for (int i = SIZE_BUCKET_COUNT - 1; i >= 0 && temp_count < MAX_CANDIDATES * 2;
       i--) {
    size_node_t *curr = params->size_buckets[i];  // 从头开始遍历，收集所有
    while (curr != NULL && temp_count < MAX_CANDIDATES * 2) {
      if (curr->obj != NULL) {
#if LOH_DEBUG_LEVEL >= LOH_DEBUG_ERROR
        if (hashtable_find_obj_id(cache->hashtable, curr->obj->obj_id) !=
            curr->obj) {
...
#endif
        temp_objs[temp_count++] = curr->obj;
      }
      curr = curr->next;
    }
  }

  // 按 obj_size 从大到小排序（简单选择排序，因为数量小）
  for (int i = 0; i < temp_count - 1 && i < max_candidates; i++) {
    int max_idx = i;
    for (int j = i + 1; j < temp_count; j++) {
      if (temp_objs[j]->obj_size > temp_objs[max_idx]->obj_size) {
        max_idx = j;
      }
    }
    if (max_idx != i) {
      cache_obj_t *tmp = temp_objs[i];
      temp_objs[i] = temp_objs[max_idx];
      temp_objs[max_idx] = tmp;
    }
  }
  ...
  LOH_DEBUG_PRINT_DETAILED("\n");
  // 从排序后的数组中选择最大的 max_candidates 个对象
  for (int i = 0; i < temp_count && added < max_candidates; i++) {
    if (start_idx + added >= MAX_CANDIDATES) break;

    cache_obj_t *obj = temp_objs[i];
