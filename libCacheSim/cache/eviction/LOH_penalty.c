...
typedef struct {
  ...
  perf_counter_t calc_score;          // calculate_score() / 评分循环
  // RL同步相关性能计时
...
//   - loh_feature_log1p=1 时：所有特征统一采用 log1p(raw)，并允许 Python
//     端直接使用原始连续动作作为权重（不再 softmax）。
...
// IRT 特征值计算：由调用方保证 irt_value 已初始化且为正
...
// RECENCY / FREQUENCY / SIZE 特征计算函数：使用 raw 值作为输入（统一使用
// int64_t raw）
...
cache_t *LOH_penalty_init(const common_cache_params_t ccache_params,
  ...
  LOH_DEBUG_PRINT_DETAILED("=== LOH Penalty Cache Initialize ===\n");
  ...
  cache_t *cache =
      cache_struct_init("LOH-penalty", ccache_params, cache_specific_params);
  cache->cache_init = LOH_penalty_init;
  ...
  if (cache_specific_params != NULL) {
    ...
    // params->weights[0] = 0.1667;
    // params->weights[1] = 0.1667;
    // params->weights[2] = 0.1667;
    // params->weights[3] = 0.1667;
    // params->weights[4] = 0.1667;
    // params->weights[5] = 0.1667;
    // LOH_DEBUG_PRINT_BASIC("[LOH INIT] Temporary: RL disabled and weights
    // fixed to [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f]\n", params->weights[0],
