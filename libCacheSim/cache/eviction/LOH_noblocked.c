...
static void sync_with_actor_critic(LOH_params_t *params) {
  ...
  // ========== 检查点1: Python是否处于训练模式 ==========
  // 如果is_training==1，说明Python正在训练，无法响应新请求
  // 直接返回，使用缓存权重继续运行，不发送状态也不等待
  if (python_training) {
    ...
    LOH_DEBUG_PRINT_BASIC(
        "[%.6f] [C-TRAINING] [seq %llu] Python in training mode "
        "(is_training=1) - using cached weights\n",
        timestamp, seq_no);
  ...
  // ========== 正常流程: 发送状态给Python ==========
  // is_training==0，可以发送状态并等待Python响应

  LOH_DEBUG_PRINT_BASIC(
      "[%.6f] [C-INFERENCE] [seq %llu] Python in inference mode "
      "(is_training=0) - sending state\n",
      timestamp, seq_no);
  ...
  // 超时策略：1秒超时，超时时检查is_training是否变为1（训练模式）

  const double timeout_sec = 1;  // 1秒超时
  const int sleep_us = 25;       // 0.025ms 轮询间隔
  int wait_cycles = 0;
  bool ack_ready = false;
  bool timeout_triggered = false;
  bool ack_via_semaphore = false;
  bool python_busy_logged = false;
  ...
  // ========== 路径1: 使用信号量等待ACK ==========
  if (params->sem_requested && params->sem_enabled) {
    const long sem_chunk_ns =
        50 * 1000000L;  // 50ms chunk，每chunk检查一次is_training
    ...
    while (true) {
      struct timespec now, chunk_deadline;
      clock_gettime(CLOCK_REALTIME, &now);
      chunk_deadline = now;
      chunk_deadline.tv_nsec += sem_chunk_ns;
      if (chunk_deadline.tv_nsec >= 1000000000L) {
        chunk_deadline.tv_sec += 1;
        chunk_deadline.tv_nsec -= 1000000000L;
      }
      // 确保chunk_deadline不超过abs_deadline
      if (chunk_deadline.tv_sec > abs_deadline.tv_sec ||
          (chunk_deadline.tv_sec == abs_deadline.tv_sec &&
           chunk_deadline.tv_nsec > abs_deadline.tv_nsec)) {
        chunk_deadline = abs_deadline;
      }

      // ========== sem_timedwait: 等待Python发送ACK信号 ==========
      int rc = sem_timedwait(params->sem_ack, &chunk_deadline);
      ...
      // --- 超时：检查is_training是否变为1（训练模式）---
      if (errno == ETIMEDOUT) {
        shm_data_t latest_state;
        read_shared_memory(params, &latest_state);
        // 如果is_training==1，说明Python进入训练模式，立即退出
        if (!python_busy_logged && latest_state.is_training == 1) {
          struct timespec timeout_time;
          clock_gettime(CLOCK_MONOTONIC, &timeout_time);
          double timeout_timestamp =
              timeout_time.tv_sec + timeout_time.tv_nsec / 1e9;

          LOH_DEBUG_PRINT_BASIC(
              "[%.6f] [C-TIMEOUT] [seq %llu] Python entered training mode "
              "during wait - using cached weights\n",
              timeout_timestamp, seq_no);
          python_busy_logged = true;
          timeout_triggered = true;
          break;
        }
        // 如果已达到最终deadline，退出（真正的超时）
        if (chunk_deadline.tv_sec == abs_deadline.tv_sec &&
            chunk_deadline.tv_nsec == abs_deadline.tv_nsec) {
          timeout_triggered = true;
          break;
      ...
      // --- 其他错误：关闭信号量，回退到轮询 ---
      LOH_DEBUG_PRINT_BASIC(
          "[LOH] sem_timedwait error (%s), disable semaphore mode\n",
          strerror(errno));
      if (params->sem_ready) {
        sem_close(params->sem_ready);
        params->sem_ready = NULL;
  ...
  if (!ack_ready && !timeout_triggered) {
    while (1) {
      ...
      if (!python_busy_logged && shm_data.is_training == 1) {
        double now_timestamp = now_ts.tv_sec + now_ts.tv_nsec / 1e9;
        LOH_DEBUG_PRINT_BASIC(
            "[%.6f] [C-POLL] [seq %llu] Python entered training mode during "
            "poll - using cached weights\n",
            now_timestamp, seq_no);
        python_busy_logged = true;
        timeout_triggered = true;
        break;
      }
  ...
  // 优化方案3: 超时时直接使用缓存权重，无需等待Python
  struct timespec timeout_time;
  clock_gettime(CLOCK_MONOTONIC, &timeout_time);
  double timeout_duration = (timeout_time.tv_sec - start_time.tv_sec) +
                            (timeout_time.tv_nsec - start_time.tv_nsec) / 1e9;

  // 超时时再次读取 is_training，检测 Python 是否已进入训练模式
  double final_timestamp = timeout_time.tv_sec + timeout_time.tv_nsec / 1e9;
  if (!python_busy_logged) {
    shm_data_t latest_state;
    read_shared_memory(params, &latest_state);
    if (latest_state.is_training == 1) {
      LOH_DEBUG_PRINT_BASIC(
          "[%.6f] [C-FINAL] [seq %llu] Final check: Python in training mode - "
          "using cached weights\n",
          final_timestamp, seq_no);
      python_busy_logged = true;
    }
  }
  if (python_busy_logged) {
    LOH_DEBUG_PRINT_BASIC(
        "[%.6f] [C-FINAL] [seq %llu] Confirmed: Python busy training - using "
        "cached weights\n",
        final_timestamp, seq_no);
  }

  LOH_DEBUG_PRINT_BASIC(
      "[%.6f] [C-TIMEOUT] [seq %llu] Final timeout after %.6f sec - using "
      "cached weights\n",
