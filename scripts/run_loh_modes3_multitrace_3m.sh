...
# Runs 3 LOH score modes across multiple traces (3M req each), then aggregates summaries.
# Modes are defined in scripts/sweep_configs_modes3_signlog1psoftmax1.txt

CACHE_RATIO=${1:-0.1}
SWEEP_REPEATS=${SWEEP_REPEATS:-1}
SEED_BASE=${SEED_BASE:-0}

CONFIGS_FILE="scripts/sweep_configs_modes3_signlog1psoftmax1.txt"
...
TS=${TS:-"modes3_$(date +%Y%m%d_%H%M%S)"}
OUT_ROOT="sweeps/${TS}"
mkdir -p "${OUT_ROOT}"
