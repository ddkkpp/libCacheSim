...
DEFAULT_MEMORY_LIMIT=90  # Default memory usage limit in percentage
...
# Usage/help
usage() {
    echo "Usage: $0 [-d DATA_DIR] [-f TRACE_FORMAT] [-a ALGORITHMS] [-p PARAMS] [-j MAX_PARALLEL] [-m MEMORY_LIMIT]"
    echo "  -d DATA_DIR      Directory containing trace files (default: ./data)"
    echo "  -f TRACE_FORMAT  Trace format (default: oracleGeneral)"
    echo "  -a ALGORITHMS    Comma-separated algorithm list (default: $DEFAULT_ALGOS)"
    echo "  -p PARAMS        Comma-separated param list (default: $DEFAULT_PARAMS)"
    echo "  -j MAX_PARALLEL  Max parallel jobs (default: $DEFAULT_MAX_PARALLEL)"
    echo "  -m MEMORY_LIMIT  Max memory usage percent (default: $DEFAULT_MEMORY_LIMIT)"
...
# Parse options
while getopts ":d:f:a:p:j:m:h" opt; do
  case $opt in
    d) DATA_DIR="$OPTARG" ;;
    f) TRACE_FORMAT="$OPTARG" ;;
    a) ALGORITHMS="$OPTARG" ;;
    p) PARAMS="$OPTARG" ;;
...
run_simulation() {
    ...
    # Execute the command, output only to log files (no extra terminal output)
...
if [ "$DEBUG_RUN_SINGLE" = true ]; then
    ...
    # Find first zst file in data directory (do not exclude any directories)
    TEST_TRACE=$(find "$DATA_DIR" -type f -name "*.zst" | head -n 1)
...
# Populate zst_files array based on -d argument (directory or single file)
declare -a zst_files
if [ -d "$DATA_DIR" ]; then
    # Directory: find all .zst files recursively
    while IFS= read -r -d $'\0' file; do
        zst_files+=("$file")
    done < <(find "$DATA_DIR" -type f -name "*.zst" -print0 | sort -z)
elif [ -f "$DATA_DIR" ] && [[ "$DATA_DIR" == *.zst ]]; then
    # Single .zst file
    zst_files=("$DATA_DIR")
else
    echo "Error: -d argument must be a directory or a .zst file."
    exit 1
fi
