...
class LohEnv(gym.Env):
    ...
    def _update_python_ready(self, value: int):
        """更新python_ready标志，优先使用字段写入，失败时回退整块写"""
        value = 1 if value else 0
        if self._write_field_int32(self._offset_python_ready, value):
            return
        data = self._read_shm()
        if not data:
            return
        data.python_ready = value
        self._write_shm(data)

    def _set_training_mode(self, enabled: bool):
        """切换训练模式，同时更新共享内存中的python_ready标志"""
        if enabled:
            if not self._training_mode.is_set():
                ts = get_monotonic_time()
                self._training_mode.set()
                self._update_python_ready(0)
                if LOH_DEBUG_BASIC():
                    print(f"[{ts:.6f}] _set_training_mode(True) - python_ready=0 written")
        else:
            if self._training_mode.is_set():
                ts = get_monotonic_time()
                self._training_mode.clear()
                self._update_python_ready(1)
                if LOH_DEBUG_BASIC():
                    print(f"[{ts:.6f}] _set_training_mode(False) - python_ready=1 written")
    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            ...
            # 但如果_training_mode已设置，则保持python_ready=0
            if data:
                try:
                    # 根据训练模式设置python_ready
                    python_ready_value = 0 if self._training_mode.is_set() else 1
                    if not self._write_field_int32(self._offset_python_ready, python_ready_value):
                        data.python_ready = python_ready_value
                        self._write_shm(data)
                except Exception:
                    pass
            waited_sem = False
        ...
        # 收到初始状态后，根据训练模式设置python_ready
        try:
            # 如果_training_mode已设置，保持python_ready=0；否则设置为1
            initial_data.python_ready = 0 if self._training_mode.is_set() else 1
        ...
        # 【关键】通过python_ready判断是否需要训练：
        # - python_ready=1: 第一个reset（初始化），不需要训练
        # - python_ready=0: episode结束后的reset，step(512)已设置为0，需要训练
        needs_training = (initial_data.python_ready == 0)

        if needs_training:
            # 这是训练后的reset，不需要发送ACK，因为C端已经在使用缓存权重
            # 也不需要设置python_ready=0，因为它已经是0
            # 只需要设置内部训练标志，并在step(1)中清除
            self._training_mode.set()
            if LOH_DEBUG_BASIC():
                ts = get_monotonic_time()
                print(f"[{ts:.6f}] Reset: python_ready=0 detected, starting training...")
        else:
            # 第一个reset（初始化），python_ready=1，不需要发送权重
            if LOH_DEBUG_BASIC():
                ts = get_monotonic_time()
                print(f"[{ts:.6f}] Reset: python_ready=1 detected (first episode), no training needed")
    ...
    def step(self, action):
        ...
        python_ready变动时刻（完整流程）：
        ┌────────────────────────────────────────────────────────────┐
        │ 时刻1: step(1)开始 - python_ready: 0→1 (训练完成)         │
        │ 时刻2-512: step()写入权重 - python_ready保持1 (推理中)    │
        │ 时刻513: reset()发送ACK - python_ready: 1→0 (训练开始)    │
        │ (SB3训练1-2秒，C端使用缓存权重继续运行)                    │
        │ 时刻1: 回到step(1)开始                                     │
        └────────────────────────────────────────────────────────────┘
        ...
        # ┌─ python_ready变动时刻1 ─┐
        # │ 如果是新episode的第一个step，恢复推理模式              │
        # │ 因为训练已在reset()返回后、step(1)调用前完成           │
        # │ python_ready: 0 → 1                                    │
        # └────────────────────────────────────────────────────────┘
        if self.current_step == 0 and self._training_mode.is_set():
            self._training_mode.clear()
            self._update_python_ready(1)
            if LOH_DEBUG_BASIC():
                print(f"[{step_start:.6f}] [Step 1] Training completed, restored python_ready=1")
        ...
        while True:
            ...
            if current.ready_for_inference == 1:
                ...
                # 根据当前训练模式设置 python_ready
                current.python_ready = 0 if self._training_mode.is_set() else 1
        ...
        while True:
            ...
            if self._sem_enabled:
                ...
                if waited_via_sem:
                    # sem 被唤醒后直接读取共享内存
                    consecutive_timeouts = 0  # 成功等到信号，重置计数器并检查是否为期待的新版本
            ...
            if timeout_counter % heartbeat_interval_checks == 0:
                ...
                elapsed = now - wait_start
                if LOH_DEBUG_VERBOSE():
                    print(f"[POLL][Python] 心跳：等待C端新状态 — 已等待 {elapsed:.1f}秒")
                # 检查空闲超时
                if (now - last_progress_t) > idle_threshold:
                    if LOH_DEBUG_BASIC():
                        print(f"⚠️  C端无响应（{now - last_progress_t:.1f}秒无活动）")
                    raise KeyboardInterrupt(f"C-side ended: no activity {now - last_progress_t:.1f}s")

        # python_ready 状态已在 ACK 阶段同步到训练/推理模式，此处无需额外处理
        ...
        # 【关键】如果episode结束（truncated），直接设置python_ready=0
        # 这是“快速训练”的关键：在step函数末尾直接通知C端，让SB3在后台高效训练
        if truncated and not terminated:
            self._update_python_ready(0)
            if LOH_DEBUG_BASIC():
                ts = get_monotonic_time()
                print(f"[{ts:.6f}] [Step {self.current_step}] Episode truncated - setting python_ready=0 (training will start)")
    ...
    #                         )
    #             if not self._training_mode.is_set() and data.python_ready != 1:
    #                 if not self._write_field_int32(self._offset_python_ready, 1):
    #                     data.python_ready = 1
    #                     self._write_shm(data)
    #             elif self._training_mode.is_set() and data.python_ready != 0:
    #                 self._update_python_ready(0)
    #         except Exception:
    #             pass
    #         time.sleep(self._bg_sleep_us / 1e6)
