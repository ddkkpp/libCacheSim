...
# 使用 compound 模型对各自 trace 在线微调（SAC_TRAIN_FREQ=256），并禁用 softmax
# - 仅当 LOH_RESUME_PATH 指定时才会 resume（loh_actor_critic_sb3.py 已强制 path-required）
# - 只做“no-softmax”这一处改动，便于对照
...
run_one() {
  ...
  echo ""
  echo "=== finetune (no-softmax): ${run_id_full} ==="
  echo "trace: ${trace}"
  echo "model: ${model}"
...
run_one "0203_ft_meta_cpd_on_meta_tf256_nosoftmax" "$META_TRACE" "$META_MODEL"

# 2) 1063 model -> 1063 trace
run_one "0203_ft_1063_cpd_on_1063_tf256_nosoftmax" "$T1063_TRACE" "$T1063_MODEL"
