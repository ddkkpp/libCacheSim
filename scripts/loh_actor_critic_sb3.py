...
class LohEnv(gym.Env):
    ...
    def step(self, action):
        ...
        """
        with GLOBAL_TIMER.time_function("env.step_total"):
    ...
    # NOTE: Previously there were __getstate__/__setstate__ helpers here to make
    # the environment appear pickleable by stripping shared-memory / semaphore
    # handles. Those helpers have been removed because they were a workaround
    # for earlier issues (local closures and other objects being reachable from
    # the model) and introduced surprising semantics: silently dropping
    # critical IPC resources on pickle could hide bugs and lead to incorrect
    # runtime behaviour when restoring the environment.

    # The correct fixes are implemented elsewhere in the codebase:
    # - `SharedMemoryData` is defined at module level (pickleable)
    # - `FunctionTimer` supports pickling of profiling stats
    # - monkey-patching of model/train/sample with local closures was removed
    #   and replaced by `ProfiledSAC` and in-method timing in ReplayBuffer
    # Together these ensure `model.save()` no longer needs the env to be
    # specially stripped. If an env instance is restored from pickle, the
    # caller should explicitly call `_attach_shared_memory()` and
    # `_init_semaphores()` to re-establish IPC resources.
...
def main():
    ...
    try:
        ...
        model = ProfiledSAC(
            ...
            learning_starts=args.learning_starts,  # 【修改】默认1000（应≥batch_size）
