...
LOG_PATH="$OUT_DIR/1063_qinput_raw_rerun_${RUN_ID}.log"
RESULTS_CSV="$OUT_DIR/results.csv"
DOC_PATH=docs/20260317-FULL_EXPERIMENT_RESULTS.md

echo "[prebuild] rebuild release with LOH_INCLUDE_HIT_MISS_FEATURES=0" >> "$OUT_DIR/runner.log"
(
  export LOH_BUILD_RELEASE=1
  export LOH_INCLUDE_HIT_MISS_FEATURES=0
  export LOH_SKIP_BUILD=0
  bash scripts/debug.sh -r
) >> "$OUT_DIR/prebuild_1063_raw_rerun.log" 2>&1 || {
  echo "[prebuild] failed, see $OUT_DIR/prebuild_1063_raw_rerun.log"
  exit 1
}

SHM_KEY=$(printf "%s" "1063_raw_rerun_$(date +%s%N)" | cksum | awk '{print $1}')
rm -f "/dev/shm/loh_ac_${SHM_KEY}" "/dev/shm/sem.loh_ac_ready_${SHM_KEY}" "/dev/shm/sem.loh_ac_ack_${SHM_KEY}"

(
  export LOH_SHM_KEY="$SHM_KEY"
  export RUN_TIMESTAMP="${RUN_ID}_1063_qinput_raw_rerun"
  ...
  export LOH_ADAPTIVE_NORM_QUANTILE_INPUT=raw
  export LOH_INCLUDE_HIT_MISS_FEATURES=0
  bash scripts/test_loh_rl_sb3.sh data/TencentCBS/1063.oracleGeneral.zst 0.1
) > "$LOG_PATH" 2>&1
RC=$?
...
echo "1063,raw,${STATUS},${RC},${FMR},${FBMR},${FMQPS},${LOG_PATH}" >> "$RESULTS_CSV"
