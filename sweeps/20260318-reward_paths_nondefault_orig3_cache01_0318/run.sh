...
OUT_DIR="tmp/reward_paths_nondefault_orig3_cache01_0318"
...
cat > "$RESULTS_CSV" <<'CSV'
path,trace,reward_mode,use_miss,miss_mode,use_trend,trend_mode,status,rc,final_mr,final_bmr,final_mqps,log_path
CSV
fi

echo "[start] reward non-default paths(9) + orig3 + cache=0.1" >> "$RUNNER_LOG"
...
already_done() {
  local path_name="$1"
  local trace_name="$2"
  grep -q "^${path_name},${trace_name},.*ok," "$RESULTS_CSV"
}
...
run_one() {
  ...
  if already_done "$path_name" "$trace_name"; then
    echo "[resume-skip] path=${path_name} trace=${trace_name} already done" >> "$RUNNER_LOG"
    return 0
  fi

  local case_name="${trace_name}_${path_name}_cache01"
  local case_stamp
  case_stamp="$(date +%m%d_%H%M%S)"
  local log_path="$OUT_DIR/${case_name}_${case_stamp}.log"
  local shm_key rc=0

  shm_key=$(printf "%s|%s|%s|%s" "$trace_name" "$path_name" "$CACHE_SIZE" "$(date +%s%N)" | cksum | awk '{print $1}')
  rm -f "/dev/shm/loh_ac_${shm_key}" "/dev/shm/sem.loh_ac_ready_${shm_key}" "/dev/shm/sem.loh_ac_ack_${shm_key}" || true

  echo "[case-start] path=${path_name} trace=${trace_name}" >> "$RUNNER_LOG"
  (
    export LOH_SHM_KEY="$shm_key"
    export LOH_REWARD_MODE="$reward_mode"
    export LOH_REWARD_USE_MISSRATIO="$use_miss"
    export LOH_MISS_COMPONENT="$miss_mode"
    export LOH_REWARD_USE_MISSRATIOTREND="$use_trend"
    export LOH_MISSRATIOTREND_MODE="$trend_mode"
  ...
  echo "${path_name},${trace_name},${reward_mode},${use_miss},${miss_mode},${use_trend},${trend_mode},${status},${rc},${fmr},${fbmr},${fmqps},${log_path}" >> "$RESULTS_CSV"
  echo "[case-done] path=${path_name} trace=${trace_name} status=${status} rc=${rc} final_mr=${fmr} final_bmr=${fbmr} final_mqps=${fmqps}" >> "$RUNNER_LOG"
...
for p in "${PATHS[@]}"; do
  ...
  for t in "${TASKS[@]}"; do
    ...
    run_one "$path_name" "$reward_mode" "$use_miss" "$miss_mode" "$use_trend" "$trend_mode" "$trace_name" "$trace_path" &
    running=$((running + 1))
    echo "[queue] active=${running}/${PARALLEL} action=start path=${path_name} trace=${trace_name}" >> "$RUNNER_LOG"
