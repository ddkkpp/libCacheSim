...
# cd: switch to repo root
ROOT_DIR=$(cd "$(dirname "$0")/../.." && pwd)
cd "$ROOT_DIR"

OUT_DIR="tmp/1063_bound_effect_0316_w0"
RESULTS_CSV="$OUT_DIR/results.csv"
RUNNER_LOG="$OUT_DIR/runner_extra3.log"
...
echo "[start] 1063 extra3 bound test (parallel=${PARALLEL}, warmup=0)" >> "$RUNNER_LOG"
...
for c in "${TASKS[@]}"; do
  ...
  run_one "$cfg_name" "$extra_envs" &
  running=$((running + 1))
  echo "[queue] active=${running}/${PARALLEL} action=start case=${TRACE_NAME}_${cfg_name}" >> "$RUNNER_LOG"
done
