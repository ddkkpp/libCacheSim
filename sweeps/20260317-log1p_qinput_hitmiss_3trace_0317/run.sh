...
echo "[start] log1p+adaptive(q_input=log1p)+hitmiss on 3 traces (parallel=${PARALLEL})" >> "$RUNNER_LOG"
...
echo "[prebuild] build release with LOH_INCLUDE_HIT_MISS_FEATURES=1" >> "$RUNNER_LOG"
(
...
TASKS=(
  "1063|data/TencentCBS/1063.oracleGeneral.zst|0.1"
  "wiki|data/WikiCDN/wiki_2019t.oracleGeneral.zst|0.1"
  "meta|data/MetaCDN/meta_reag.oracleGeneral.zst|0.1"
)
...
run_one() {
  ...
  local trace_path="$2"
  local cache_size="$3"
  local case_name="${trace_name}_log1p_qinput_log1p_hitmiss"
  local case_stamp
...
running=0
for t in "${TASKS[@]}"; do
  IFS='|' read -r trace_name trace_path cache_size <<< "$t"
  run_one "$trace_name" "$trace_path" "$cache_size" &
  running=$((running + 1))
