...
cache_t *LOH_init(const common_cache_params_t ccache_params,
  ...
  // 3.7) 自适应候选预算配置
  {
    ...
    if (loh_adaptive_budget) {
      printf("[LOH INIT] Adaptive budget ENABLED: rebalance period=%d\n",
             LOH_ADAPTIVE_REBALANCE_PERIOD);
    }
  }
...
scoring_done:  // ultra_fast 路径跳转到这里
  ...
  if (loh_adaptive_budget && best_i >= 0 && best_i < params->n_candidates) {
    ...
    if (loh_adaptive_total_evict % LOH_ADAPTIVE_REBALANCE_PERIOD == 0) {
      ...
      // 与旧方案不同：只在 winner 确定后记录 1 次得分（O(1)/驱逐），
      //   而非在评分循环中对每个候选累加（旧方案 O(n)/驱逐，~192 次/驱逐）
      ...
      if (have_stats && global_max_avg > global_min_avg) {
        // effectiveness = (max_avg - avg_score[s]) / (max_avg - min_avg)
        // → 得分最低的源 effectiveness=1.0，最高的=0.0
        ...
        if (total_eff > 0) {
          ...
          for (int s = 0; s < n_active; s++) {
            loh_source_budget[s] =
                (int)(effectiveness[s] / total_eff * loh_structured_candidates +
                      0.5);
      ...
      // else: 无统计或所有源平均得分相同 → 回退到 win-count 分配
      else if (have_stats) {
