#!/bin/bash
# Random-Only RL sweep (refined): MAX_CAND in {64, 128, 192}, 3 traces, 8M, serial
set -e
...
run_test() {
  ...
  echo "  ${result}"
  echo "${trace_name},${max_cand},${result}" >> tmp/randonly_sweep_refined.csv
}

echo "trace,max_cand,raw_result" > tmp/randonly_sweep_refined.csv

for cand in 64 128 192; do
  # Meta (LOG1P, fastest)
...
cat tmp/randonly_sweep_refined.csv
