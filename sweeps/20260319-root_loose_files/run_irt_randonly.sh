#!/bin/bash
# compound=0, irt=1, 纯随机候选 RL 测试
# 6 维特征: recency, freq, size, irt1, irt2, irt3
# CAND=96, 8M requests, 3 traces
set -e
...
for spec in "data/MetaCDN/meta_reag.oracleGeneral.zst meta 1 0" "data/TencentCBS/1063.oracleGeneral.zst 1063 1 0" "data/WikiCDN/wiki_2019t.oracleGeneral.zst wiki 0 1"; do
  ...
  logfile="tmp/rl_${name}_irt_randonly_8M.log"
  echo "=== [$(date '+%H:%M:%S')] Starting: ${name} IRT+RandOnly (compound=0, irt=1) ==="
  ...
  LOH_SCORE_USE_COMPOUND=0 LOH_SCORE_USE_IRT=1 \
...
echo "=== ALL IRT+RANDONLY TESTS DONE ==="
