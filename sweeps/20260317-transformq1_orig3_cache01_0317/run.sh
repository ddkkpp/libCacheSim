...
OUT_DIR="tmp/transformq1_orig3_cache01_0317"
...
echo "[start] transform_quantile=1 + orig3 + cache=0.1 + hitmiss=0" >> "$RUNNER_LOG"
...
run_one() {
  local trace_name="$1"
  local trace_path="$2"
  local case_name="${trace_name}_transformq1_cache01"
  local case_stamp
