...
OUT_DIR="tmp/transformq0_hitmiss1_orig3_cache01_0317"
...
echo "[start] transform_quantile=0 + hitmiss=1 + orig3 + cache=0.1" >> "$RUNNER_LOG"

echo "[prebuild] force release + LOH_INCLUDE_HIT_MISS_FEATURES=1" >> "$RUNNER_LOG"
(
...
run_one() {
  local trace_name="$1"
  local trace_path="$2"
  local case_name="${trace_name}_transformq0_hitmiss1_cache01"
  local case_stamp
  case_stamp="$(date +%m%d_%H%M%S)"
  local log_path="$OUT_DIR/${case_name}_${case_stamp}.log"
  local shm_key rc=0

  shm_key=$(printf "%s|%s|%s" "$trace_name" "$CACHE_SIZE" "$(date +%s%N)" | cksum | awk '{print $1}')
