...
BEGIN = "<!-- AUTO_LOG1P_QINPUT_4TRACE_0317_BEGIN -->"
END = "<!-- AUTO_LOG1P_QINPUT_4TRACE_0317_END -->"
...
def build_block(csv_path, rows):
    ...
    lines = [
        ...
        "",
        "| Trace | Raw->Out | Norm | Status | RC | Final MR | Byte MR | MQPS | Log |",
        "|---|---|---|---|---:|---:|---:|---:|---|",
    ]
    for row in sorted(rows, key=lambda r: r.get("trace", "")):
        ...
        lines.append(
            "| {} | `log1p` | `adaptive(lo=0.0, hi=1.0, warmup=0, q_input=log1p)` | {} | {} | {} | {} | {} | `{}` |".format(
                row.get("trace", ""),
                row.get("status", ""),
                row.get("rc", ""),
