...
OUT_DIR="tmp/log1p_qinput_4trace_0317"
...
LOH_INCLUDE_HIT_MISS_FEATURES=0 LOH_BUILD_RELEASE=1 bash scripts/debug.sh -r -c >> "$OUT_DIR/prebuild_rel.log" 2>&1
...
echo "[start] log1p+adaptive(q_input=log1p) 4 traces parallel=${PARALLEL}" >> "$RUNNER_LOG"
...
TRACES=(
  "Alibaba-4|data/Alibaba/alibabaBlock_4.oracleGeneral.zst"
...
run_one() {
  local trace_name="$1"
  local trace_path="$2"
  local case_name="${trace_name}_log1p_qinput_log1p"
