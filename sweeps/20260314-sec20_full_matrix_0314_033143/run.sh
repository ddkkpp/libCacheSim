...
OUT_DIR="tmp/sec20_full_matrix_0314_033143"
...
extract_metric_pair() {
  ...
  # Preferred: explicit summary lines from test script.
  ...
  # Fallback: final cachesim line contains "miss ratio X, byte miss ratio Y".
  if [ -z "${mr}" ] || [ -z "${bmr}" ]; then
    local last_line
...
run_one() {
  ...
  local shm_key
  shm_key=$(printf "%s|sec20full|%s" "$case_name" "$OUT_DIR" | cksum | awk '{print $1}')
