...
OUT_DIR="${OUT_DIR:-tmp/td3_ex0_retest_0318}"
mkdir -p "$OUT_DIR"
...
SEED1="${SEED1:-$((RANDOM + 20260000))}"
SEED2="${SEED2:-$((RANDOM + 20270000))}"

RUNNER_LOG="$OUT_DIR/two_repeats_runner.log"
CSV_PATH="$OUT_DIR/two_repeats_results.csv"
...
run_case() {
  ...
  local case_name="${trace_name}_TD3_ex0_cache01_${run_tag}_${stamp}"
...
echo "[start] td3_ex0 two repeats parallel reqs=${REQS} cache=${CACHE_SIZE} seed1=${SEED1} seed2=${SEED2}" >> "$RUNNER_LOG"
...
echo "[finished] td3_ex0 two repeats parallel" >> "$RUNNER_LOG"
