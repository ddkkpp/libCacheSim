...
OUT_DIR="tmp/sac_ex0_orig3_seed42_3407_0318"
mkdir -p "$OUT_DIR"
...
if [ ! -f "$CSV_PATH" ]; then
  cat > "$CSV_PATH" <<'CSV'
run_tag,trace,algo,seed,exclude_recent_steps,final_mr,final_bmr,final_mqps,log_path,rc
CSV
fi
...
run_case() {
  ...
  local case_name="${trace_name}_SAC_ex0_${run_tag}_${stamp}"
  local log_path="$OUT_DIR/${case_name}.log"
  local rc=0

  local shm_key
  shm_key=$(printf "%s|%s|%s|%s|%s" "$trace_name" "SAC" "0" "$CACHE_SIZE" "${seed}_${run_tag}_${stamp}" | cksum | awk '{print $1}')
  ...
  echo "${run_tag},${trace_name},SAC,${seed},0,${fmr},${fbmr},${fmqps},${log_path},${rc}" >> "$CSV_PATH"
...
echo "[start] sac ex0 orig3 two seeds reqs=${REQS} cache=${CACHE_SIZE} seeds=${SEED1},${SEED2}" >> "$RUNNER_LOG"
...
echo "[finished] sac ex0 orig3 two seeds" >> "$RUNNER_LOG"
