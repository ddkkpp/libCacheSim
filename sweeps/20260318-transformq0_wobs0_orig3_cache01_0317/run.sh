...
OUT_DIR="tmp/transformq0_wobs0_orig3_cache01_0317"
...
echo "[start] transform_quantile=0 + weights_in_obs=0 + hitmiss=0 + orig3 + cache=0.1" >> "$RUNNER_LOG"
...
run_one() {
  local trace_name="$1"
  local trace_path="$2"
  local case_name="${trace_name}_transformq0_wobs0_cache01"
  local case_stamp
