...
SRC_DIR = ROOT / "docs/20260422-ablation-per-group-cache01/cmaes版本LOH"
BASELINE_MD = ROOT / "docs/20260422-ablation-per-group-cache01/cmaes版本LOH/baseline_summary_v7_3lcache-target.md"
ROBUSTNESS_MD = ROOT / "docs/20260422-ablation-per-group-cache01/cmaes版本LOH/robustness_summary_v7_3lcache-target.md"
OUT_DIR = Path(__file__).resolve().parent
DOC_OUT = ROOT / "docs/20260506-ROBUSTNESS_PLOTS_STATS-cache01-v7-3lcache-target.md"
...
def parse_group(group_name: str):
    ...
    loh_mr = {}
    loh_bmr = {}
    one_hit = {}  # trace -> one_hit value (from 表1)
    ...
    for i, line in enumerate(lines):
        if line.startswith("## 表1"):
            ...
            if "trace" in idx and "n_req" in idx:
                for r in rows:
                    ...
                    # read one_hit (column name differs from cache0001's one_hit_ratio)
                    oh_col = "one_hit" if "one_hit" in idx else "one_hit_ratio"
                    if oh_col in idx:
                        v = parse_num(r[idx[oh_col]])
                        if not np.isnan(v):
                            one_hit[tr] = v
        ...
        if line.startswith("## 表2"):
            h, rows = parse_table_after(lines, i)
            idx = {k: j for j, k in enumerate(h)}
            # v7 rule: MR = f001_orig, fallback f100_ns
            if "trace" in idx and ("f001_orig" in idx or "f100_ns" in idx):
                for r in rows:
                    tr = r[idx["trace"]]
                    if tr.startswith("**"):
                        continue
                    v = np.nan
                    if "f001_orig" in idx:
                        v = parse_num(r[idx["f001_orig"]])
                    if np.isnan(v) and "f100_ns" in idx:
                        v = parse_num(r[idx["f100_ns"]])
                    loh_mr[tr] = v
            elif "trace" in idx and "best_LOH" in idx:
                # fallback: use precomputed best_LOH column if f001_orig not present
                for r in rows:
                    tr = r[idx["trace"]]
                    if tr.startswith("**"):
                        continue
                    loh_mr[tr] = parse_num(r[idx["best_LOH"]])

        if line.startswith("## 表3"):
            h, rows = parse_table_after(lines, i)
            idx = {k: j for j, k in enumerate(h)}
            # v7 rule: BMR = one_hit<=0.72 -> f100_ns; else f101_orig; fallback f100_ns
            if "trace" in idx and ("f100_ns" in idx or "f101_orig" in idx):
                for r in rows:
                    tr = r[idx["trace"]]
                    if tr.startswith("**"):
                        continue
                    oh = one_hit.get(tr, 0.0)
                    v = np.nan
                    if oh <= 0.72:
                        if "f100_ns" in idx:
                            v = parse_num(r[idx["f100_ns"]])
                        if np.isnan(v) and "f101_orig" in idx:
                            v = parse_num(r[idx["f101_orig"]])
                    else:
                        if "f101_orig" in idx:
                            v = parse_num(r[idx["f101_orig"]])
                        if np.isnan(v) and "f100_ns" in idx:
                ...
                for r in rows:
                    ...
                    if tr.startswith("**"):
                        continue
                    loh_bmr[tr] = parse_num(r[idx["best_LOH"]])
        ...
        if line.startswith("## 表4"):
            ...
            for r in rows:
                ...
                # always overwrite best_LOH with v7-computed value
                mr[tr]["best_LOH"] = loh_mr.get(tr, np.nan)
        ...
        if line.startswith("## 表5"):
            ...
            for r in rows:
                ...
                # always overwrite best_LOH with v7-computed value
                bmr[tr]["best_LOH"] = loh_bmr.get(tr, np.nan)
