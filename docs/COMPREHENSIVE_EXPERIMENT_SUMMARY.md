...
> 数据来源：75 个 sweep 目录、346 次 RL 运行(docs/RL_CacheSim_Run_Summary_by_Trace.md)、8 个常量权重搜索目录、.tmp_*_table.md 文件、多份阶段文档
...
- 来源：LOH_CONSTANT_WEIGHTS_BEST_3M_WIKI2019T_1063.md（第一节）
- 最优：W=[1,1,1.5,0,0,0] → 0.3525（refine）, coarse: W=[1,1,1,0,0,0] → 0.3723

### 3.3 compound=1 + mrw=1 + softmax=1 + IRT=1 + LOG1P=1（2026-02-01）
- 来源：LOH_CONSTANT_WEIGHTS_BEST_3M_WIKI2019T_1063.md（第二节）、runs/constant_weight_norl_v012c1_mrw1…
...
| OMR | 权重 | 配置 | 来源 |
|-----|------|------|------|
| **0.3680** | [0,0,1,0,0,0] | LOH-mr-blocked, no LOG1P, no compound | docs/sweep_results.md |
...
| 0.5992 | [3,1,4,0,0,0] | no LOG1P, no compound, refine | docs/sweep_results.md |
...
| 文件 | 内容概要 |
|------|---------|
| docs/RL_CacheSim_Run_Summary_by_Trace.md | 346 次 RL 运行按 trace 分组（11490 行） |
...
| LOH_CONSTANT_WEIGHTS_BEST_3M_WIKI2019T_1063.md | 固定权重搜索+Teacher优化（231 行） |
| LOH_TESTED_CONFIGS_SUMMARY.md | 自动生成的 sweep 索引（5715 行） |
| docs/sweep_results.md | LOH-mr-blocked 固定权重 Top20（172 行） |
| docs/LOH_Framework_and_Experiments_Summary.md | 框架与变种总览 |
| 20260215_1063_to032_阶段总结与完成度.md | Stage1-3 进度 |
