...
- `20251028-20251028-README.md`：简要说明 `scripts/` 目录中脚本的用途或使用方式。
...
- `20250729-20250729-LOH_SHARED_MEMORY_FIX.md`：记录 LOH 共享内存结构（`shm_data_t`）修复过程，包括字段对齐、大小校验和 Python ctypes 映射问题。
- `20250729-20250729-loh_file_shm_patch.md`：描述针对 LOH 共享内存文件的补丁方案和具体修改步骤（如 SHM 名称、权限等）。
- `20250808-20250808-LOH_DEEP_ANALYSIS_FIXES.md`：深入分析 LOH 早期实现中的 bug 和逻辑问题，并给出修复方案。
- `20250807-20250807-LOH_IMPROVEMENTS_SUMMARY.md`：总结一系列针对 LOH 算法和实现的改进点（数据结构修复、统计逻辑调整等）。
- `20250902-20250902-LOH_FIX_SUMMARY.md`：集中列出 2024-09-02 前后对 LOH 实现进行的修复列表及影响范围。
- `20250814-20250814-LOH_Algorithm_Architecture.md`：详细说明 LOH 算法的整体架构、状态向量定义、特征设计和在线 RL 集成方式，是核心设计文档。
- `20250925-20250925-LOH_REWARD_WEIGHTS_FEATURE.md`：说明 miss_ratio/byte_miss_ratio 奖励权重的设计和配置方式，以及 C/Python 之间的参数传递。
- `20251016-20251016-LOH_INFERENCE_GUIDE.md`：指导如何在“只推理”模式下使用 LOH（`loh_inference_only.py`），包括部署和调试建议。
- `20251023-20251023-PENALTY_FIX_SUMMARY.md`：总结 penalty 相关 bug 和修复（如距离度量、队列维护），与 RL 惩罚机制紧密相关。
- `20251023-20251023-PENALTY_MECHANISM_REDESIGN.md`：讨论 penalty 机制的重新设计方案，使其与 miss ratio 量纲对齐，并给出多个实现选项。
- `20251022-20251022-SAC_RETROSPECTIVE_GUIDE.md`：介绍 SAC + 事后惩罚（RetrospectiveReplayBuffer）的设计思想、实现细节和调参建议。
- `20251023-20251023-SAMPLING_EXCLUSION_STRATEGY.md`：说明在经验回放中排除某些样本（如最近样本）的策略，以提高训练稳定性和样本多样性。
- `20251022-20251022-TEST_RETROSPECTIVE_PENALTY.md`：记录 retrospective penalty 相关的测试用例、实验结果和问题分析。
- `20251023-20251023-TUNING_GUIDE.md`：提供 LOH RL 系统的调参指南，包括 buffer_size、learning_starts、train_freq 等超参及其联动关系。
- `20251023-20251023-EVICTION_TIMESTAMP_IMPROVEMENT.md`：记录关于驱逐时间戳(`eviction_timestamp`)的改进方案，以便更精确地计算 penalty 和重访间隔。
- `20251023-20251023-PENALTY_FIXES_SUMMARY.md`：汇总 2024-10-23 左右对 penalty 逻辑的几次修复及其对实验结果的影响。
- `20251023-20251023-TWO_COMPONENT_PENALTY_IMPLEMENTATION.md`：描述“两组件 penalty”实现方案，将 penalty 拆分为多个部分以更精细地惩罚不同类型错误。
- `20251023-20251023-TWO_COMPONENT_PENALTY_TEST_REPORT.md`：针对两组件 penalty 的测试报告，包含实验配置和结果对比。
- `20251024-20251024-CTRL_C_BEHAVIOR.md`：说明在使用 Ctrl+C 中断时 C/Python 进程与 IPC 资源应如何处理，避免残留共享内存和信号量。
- `20251024-20251024-LOH_GRACEFUL_SHUTDOWN.md`：描述 LOH 系统优雅关闭的设计，包括 terminate 标志、信号处理和日志刷盘策略。
- `20251027-20251027-PENALTY_QUEUE_OPTIMIZATION.md`：讨论 penalty 队列的数据结构和性能优化方案，以减少 RL 周期内的开销。
- `20251028-20251028-REWARD_IMPROVEMENT_PLAN.md`：提出改进 reward 设计的计划和思路，包含新的 reward 组件和评估指标。
- `20251114-LOH_ENV_VARS.md`：系统列出所有 LOH 相关环境变量，说明其含义、默认值和对 C/Python 行为的影响，是配置参考必读文档。
- `20251117-20251117-LOH_EXP_HISTORY.md`：按时间顺序记录 LOH 实验与实现演进历史，包括关键 bug 修复、特性添加和重要实验现象。
- `20251117-20251117-LOH_Framework_and_Experiments_Summary.md`：当前 LOH 算法框架与 C/Python 变种 + 实验问题的总览文档，概括整体设计和主要实验结论。
- `20251028-20251028-README.md`：`docs` 目录的总体说明，指引读者如何查找各专题文档。
- `RL_CacheSim_Run_Summary.cache.json`：缓存 RL + cachesim 各次 run 的摘要 JSON，供分析脚本快速读取（如按 trace 聚合结果）。
- `20251107-20251107-RL_CacheSim_Run_Summary_by_Trace.md`：按 trace 维度汇总 RL + cachesim 的运行结果与关键指标，方便对比不同算法和配置。

---

## 3. 使用建议

- 若要了解 **LOH 算法逻辑与架构**，优先阅读：`20250814-20250814-LOH_Algorithm_Architecture.md` + `20251117-20251117-LOH_Framework_and_Experiments_Summary.md`。
- 若要排查 **共享内存/penalty/奖励相关问题**，重点参考：`20250729-20250729-LOH_SHARED_MEMORY_FIX.md`、`20251023-20251023-PENALTY_MECHANISM_REDESIGN.md`、`20251022-20251022-SAC_RETROSPECTIVE_GUIDE.md`、`10232259-*PENALTY*` 系列文档。
- 若要运行或扩展 **RL 实验脚本**，建议先看 `scripts/test_loh_rl_sb3.sh` 以及 `20251114-LOH_ENV_VARS.md`，再根据需要选择对应的 `loh_actor_critic_sb3*.py` 变体。
