...
- `LOH_ENABLE_ADAPTIVE_FEATURE_NORMALIZATION`（仅在 `LOH.c` 中，默认: `0`）
  - 设为 `1/true` 时，启用基于在线分位数的自适应归一化。
  - 归一化作用于“变换后的特征值”，不是 raw 特征值。
  - 当该开关开启时，优先于固定 `LOH_FEATURE_NORM_MAX_*` 归一化路径。

- `LOH_ADAPTIVE_NORM_LO_Q` / `LOH_ADAPTIVE_NORM_HI_Q` / `LOH_ADAPTIVE_NORM_WARMUP`
  - 分别控制自适应归一化的低分位、高分位和预热样本数。
  - 默认值为 `0.01 / 0.99 / 4096`。
  - 预热阶段会先返回未缩放的变换值；达到 warmup 后才按分位数区间映射到 `[0,1]`。
